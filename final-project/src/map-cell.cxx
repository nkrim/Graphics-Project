...
void Cell::InitTextures (View *view) {
  // load textures
    ...
    if (this->_map->hasNormalMap()) {
       ...
       // init height
        cs237::image2d *img = new cs237::image2d( this->Datafile("/hf.png").c_str() );
        _height = new cs237::texture2D(GL_TEXTURE_2D, img);

        _height->Parameter (GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        _height->Parameter (GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    ...
    if (this->_map->hasWaterMask()) {
        ...
        cs237::image2d *img2 = new cs237::image2d( this->Datafile("/water.png").c_str() );
        _water = new cs237::texture2D(GL_TEXTURE_2D, img2);

        _water->Parameter (GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        _water->Parameter (GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    }
