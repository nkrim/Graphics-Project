...
class Cell {
  ...
  // constants
    ...
    cs237::texture2D *_water;
    cs237::texture2D *_height;
...
class Tile {
  ...
  //! the width of this cell in hScale units.  The number of vertices across will be width()+1
    ...
    uint32_t CellWidth() const {return this->_cell->Width(); }
  ...
  // dump the tile tree to an output stream
    ...
    class Texture *colTex;
    class Texture *normTex;
    ...
    Cell *TileCell () {
      return _cell;
        ...
        // Pointer for vao 
    struct VAO *vao;
    // Boolean for telling UpdateVAOs function that this tile is in use
    bool inUse;



    // Morph variables
    // bound between [-1, 1], multiplied into morphDelt to get new vertex height, sign shows direction (positive is going to parent)
    // 0 means no morphing and wasn't previously in scene, 1 means it has morphed, -1 means it is currently morphing from it's child
    float tDelt;
