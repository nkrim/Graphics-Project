...
class Camera {
  ...
  //! the horizontal field-of-view of the view frustum
    ...
    float radHalfFOV () const { return this->_halfFOV; }
  ...
  //! a transformation matrix that assumes that the camera is at the origin.
    cs237::mat4x4f originViewTransform () const;
  ...
  //! \return the screen-space error
    ...
    cs237::vec3f MoveForwards(float amount);
    cs237::vec3f MoveRight(float amount);
    cs237::vec3f MoveUp(float amount);
    cs237::vec3f MoveInUp(float amount);


    void enforceValidMovement(cs237::vec3f newPos);
